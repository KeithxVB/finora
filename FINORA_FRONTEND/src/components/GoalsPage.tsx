...
export function GoalsPage({ goals, onAddGoal, onUpdateGoal, onDeleteGoal }: GoalsPageProps) {
  useEffect(() => {
    goalService.getAll()
      .then(setGoals)
      .catch(() => toast.error('Error al cargar las metas'));
  }, []);
  ...
  const handleAddContribution = (goalId: string) => {
    ...
    interface UpdateGoalResponse {
      ...
      // Add other required fields from Goal interface
    }

    if (amount > 0) {
      goalService.update(goalId, { currentAmount: (goals.find(g => g.id === goalId)?.currentAmount || 0) + amount })
        .then((updatedGoal: UpdateGoalResponse) =>
          setGoals((prev: Goal[]) => prev.map((g) => (g.id === goalId ? updatedGoal as Goal : g)))
        )
        .then(() => toast.success('¡Aporte agregado exitosamente!'))
        .catch(() => toast.error('Error al agregar aporte'));
    ...
    interface CreateGoalResponse extends Goal {
      id: string;
      userId: string;
    }

        goalService.create<CreateGoalRequest, CreateGoalResponse>({
          title: newGoal.title,
          targetAmount,
          currentAmount: 0,
          ...
          .then((createdGoal: CreateGoalResponse) => setGoals((prev: Goal[]) => [...prev, createdGoal]))
          .then(() => toast.success('¡Meta creada exitosamente!'))
  ...
  const handleDeleteGoal = (goalId: string, goalTitle: string) => {
    ...
    .then(() => toast.success(`Meta "${goalTitle}" eliminada`))
    .catch(() => toast.error('Error al eliminar la meta'));
...
function setGoals(value: Goal[]): Goal[] | PromiseLike<Goal[]> {
